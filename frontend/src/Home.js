...
function Home({ clientId: propClientId, userRole }) {
  ...
  useEffect(() => {
    ...
    const fetchMode = async () => {
      try {
        const response = await fetch(`${API_URL}mode/?client_id=${clientId}`, {
          headers: { "Authorization": `Token ${localStorage.getItem('token') || ''}` },
        });
        if (response.ok) {
          const data = await response.json();
          const mode = data.message.toLowerCase();
          setArmedHomes(client.homes.map(() => mode !== 'disarm'));
          setStayMode(client.homes.map(() => mode === 'stay'));
        }
      } catch (error) {
  ...
  const toggleHomeArmStatus = async (index) => {
    const newArmedHomes = [...armedHomes];
    newArmedHomes[index] = !newArmedHomes[index];
    setArmedHomes(newArmedHomes);
    const mode = newArmedHomes[index] ? (stayMode[index] ? "stay" : "away") : "disarm";
  ...
  const toggleStayMode = async (index) => {
    ...
    const mode = newStayMode[index] ? "stay" : "away";
  ...
  if (!isValidClient) return <div className="home-container"><p>Invalid Client ID. Please check the URL.</p></div>;
  if (!client) return <div className="home-container">Loading...</div>;
        ...
        {client.homes.map((home, index) => (
          <div key={home.siteId} className="home-card">
            <div className="home-header">
              <img src={Logo1} alt={`Home ${index + 1} Logo`} className="logo-img" />
              <div className="home-details">
                ...
                <p className="status-text">
                  Status: <span className={`status-indicator ${armedHomes[index] ? "status-armed" : "status-disarmed"}`}>{armedHomes[index] ? "Armed" : "Disarmed"}</span>
                </p>
                <p className="stay-status">
                  Mode: <span className={`status-indicator ${stayMode[index] ? "status-stay" : "status-away"}`}>{stayMode[index] ? "Stay" : "Away"}</span>
                </p>
            ...
            <div className="home-actions">
              <button className="toggle-btn" onClick={() => toggleHomeArmStatus(index)}>{armedHomes[index] ? "Disarm" : "Arm"}</button>
              <button className="mode-btn" onClick={() => toggleStayMode(index)}>{stayMode[index] ? "Away" : "Stay"}</button>
              <button className="view-btn" onClick={() => navigate(`/security-system/${home.siteId}`)}>Security</button>
              <button className="view-btn" onClick={() => navigate(`/site-data/${home.siteId}`)}>Site Data</button>
            </div>
          </div>
        ))}
      </div>
