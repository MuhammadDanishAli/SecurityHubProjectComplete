...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        client.subscribe("security/mode/ack")
        client.subscribe("security/heartbeat")
        client.subscribe("security/emergency")
...
def on_message(client, userdata, message):
    ...
    try:
        ...
        if topic.startswith("security/sensors/"):
            ...
            Sensor = getattr(api.models, 'Sensor')
            SensorData = getattr(api.models, 'SensorData')

            # Store data in database
            sensor, created = Sensor.objects.get_or_create(
                node_id=sensor_id,
                defaults={'location': location, 'sensor_type': sensor_type, 'client_id': '1'}
            )
        ...
        elif topic == "security/mode/ack":
            # Handle mode acknowledgment from ESP32
            client_id = payload.get('client_id', 'unknown')
            mode = payload.get('mode', 'unknown')
            print(f"Mode acknowledgment received: client {client_id}, mode {mode}")
            async_to_sync(channel_layer.group_send)(
                "sensors",
                {
                    "type": "sensor_update",
        ...
        elif topic == "security/heartbeat":
            # Handle heartbeat messages
            client_id = payload.get('client_id', 'unknown')
            status = payload.get('status', 'unknown')
            mode = payload.get('mode', 'unknown')
            print(f"Heartbeat received: client {client_id}, status {status}, mode {mode}")
            sensor_last_seen[client_id] = datetime.now().timestamp()
            check_disconnected_sensors()

        elif topic == "security/emergency":
            # Handle emergency alerts
            client_id = payload.get('client_id', 'unknown')
            sensor = payload.get('sensor', 'unknown')
            message = payload.get('message', 'unknown')
            print(f"Emergency alert: client {client_id}, sensor {sensor}, message {message}")
            Sensor = getattr(api.models, 'Sensor')
            AlertLog = getattr(api.models, 'AlertLog')
            sensor_obj, _ = Sensor.objects.get_or_create(
                node_id=sensor,
                defaults={'location': 'unknown', 'sensor_type': 'unknown', 'client_id': client_id}
            )
