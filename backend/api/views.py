...
class SensorStatusView(APIView):
    ...
    def get(self, request):
        ...
        for sensor in sensors:
            latest_data = sensor.data_points.order_by('-timestamp').first()
            data[sensor.node_id] = {
                'connected': sensor.status == 'active',
...
class ModeView(APIView):
    ...
    def post(self, request):
        ...
        mqtt_payload = f'{{"mode":"{mode.capitalize()}"}}'
        topic = "security/mode"
        try:
            print(f"Publishing mode {mode} to {topic} for client {client_id}")
            ...
            print(f"Failed to publish MQTT mode change: {e}")
            return Response({
                'status': 'error',
